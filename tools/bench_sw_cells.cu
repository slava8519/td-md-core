// SW-ladder T5b — the cell-list culling bake-off + the SW entry of the angular flagship (10⁶)
// atom-steps/s table. Times the heavy SW force kernel (φ₂ + φ₃ transpose-replay) all-window vs
// cell-list culled, on a jittered diamond-Si lattice, reporting R_cull = t(all-window)/t(cells)
// per N, the realized AUTO cell_div, the realized kMaxNbr, and the atom-steps/s at the largest N.
// ISOLATED-kernel throughput (free-z / z=1 / fp64) — NOT the live ring (host-orchestrated,
// concurrency-dead at z=1), NOT a bitwise-to-EAM comparison; quote it INTERNAL (NOT EAM-comparable).
// SW is int64-ORDER-FREE ⇒ cells == all-window BITWISE BY B1 (no canonical sort, the EAM-like proof).
//   build: -DTDMD_WITH_CUDA=ON ; run: ./bench_sw_cells [--n 16384] [--reps 8] [--flagship] [--cellk K]
...
#include "tdmd/cuda/zone_sw.cuh"        // all-window SW force kernel
...
int main(int argc, char** argv) {
  ...
  std::printf("bench_sw_cells: %s, %d SMs, %.1f GiB\n", pr.name, pr.multiProcessorCount,
  ...
  for (int N : sizes) {
    ...
    long long* d_pe = up(z1); long long* d_ntri = up(z1);
    ...
    // AUTO cell_div (the same heuristic as GpuSwWindowState::ensure_grid_geometry) — or override.
    ...
    tdcu::SwCellGrid grid = tdcu::sw_build_window_grid(dwx, dwy, dwz, m, box_lo, box_len, per,
                                                       sp.rcut(), cell_div);

    const int ng = (m + blk - 1) / blk;
    cudaEvent_t e0, e1; cudaEventCreate(&e0); cudaEventCreate(&e1);
    // The all-window SW force is O(m²) per thread with an INNER O(m) wing re-scan ⇒ ~O(m³) total —
    // intractable above a few thousand (that IS why no angular flagship exists). Cap the all-window
    ...
    const double t_aw = do_aw ? time_aw([&]{
      tdcu::sw_force_kernel<<<ng, blk>>>(dwx, dwy, dwz, dkey, m, downed, m, geom, sp,
          d_fx, d_fy, d_fz, d_pe, d_ntri, d_mr, d_of); }) : 0.0;
    const double t_cl = time_cl([&]{
      tdcu::sw_force_cells_kernel<<<ng, blk>>>(dwx, dwy, dwz, dkey, m, downed, m, geom, sp,
          grid.g, grid.d_starts, grid.d_counts, grid.d_order, d_fx, d_fy, d_fz, d_pe, d_ntri,
          d_mr, d_of); });
    ...
    tdcu::sw_cells_free(grid);
    for (void* q : {(void*)dwx,(void*)dwy,(void*)dwz,(void*)dkey,(void*)downed,(void*)d_fx,
                    (void*)d_fy,(void*)d_fz,(void*)d_pe,(void*)d_ntri,(void*)d_mr,(void*)d_of})
      cudaFree(q);
    cudaEventDestroy(e0); cudaEventDestroy(e1);
  }
  std::printf("\nNOTE: a-steps/s is the ISOLATED SW force kernel throughput (free-z/z=1/fp64), the SW\n"
              "entry of the angular flagship table — NOT the live ring (concurrency-dead at z=1) and\n"
              "NOT a bitwise EAM comparison. R_cull = t(all-window)/t(cells). SW is int64-ORDER-FREE ⇒\n"
              "cells == all-window BITWISE by B1 (no canonical sort).\n");
  return 0;
}
