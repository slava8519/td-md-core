# E5b — EAM на GPU-кольце: состязательный дизайн

**СТАТУС: ВЫПОЛНЕН 2026-06-17** (E5b-1 F4 → E5b-2 одно-узловой ≡ serial → E5b-3a оракул-гейт → E5b-3b стриминговое z>1 кольцо ≡ CPU `EamRing` bitwise + независимые fp64-оракулы free+PBC). Реализовано **policy-подходом** (переиспользование проверенной оркестровки `EamRing` + GPU-политика window-force), НЕ отдельным GPU-resident транспортом — последний (perf) отложен в E5c вместе с EAM cell-lists/verlet + EAM-бейкофф. `Test_CUDA_EAM_Ring` (10 тестов). Ниже — исходный дизайн (часть реализована иначе, см. статус).
