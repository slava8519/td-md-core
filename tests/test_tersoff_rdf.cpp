// M6 / Tersoff-ladder Te7: statistical RDF — the time-averaged g(r) from our Tersoff NVE
// trajectory (started from a LAMMPS-equilibrated 600 K diamond-Si config) cosine-overlaps
// LAMMPS's time-averaged g(r) (the WHOLE curve). g(r) is an equilibrium ensemble average:
// though our trajectory Lyapunov-diverges from LAMMPS's, the AVERAGED curves coincide. CI is
// LAMMPS-free (frozen reference_data/tersoff_si/). Closes the Tersoff suite (forces golden = Te1
// `Test_Tersoff_LAMMPS`; structure = here). Mirrors Test_SW_RDF / EAM E7.
...
void serial_vv_tersoff(core::AtomSoA<double>& a, const core::PairGeom& geom,
                       const pot::TersoffParams& p, long steps, double dt) {
  core::zero_forces(a); pot::tersoff_run_fixed(a, geom, p);
...
TEST(TersoffRdf, StatisticalOverlapMatchesLammps) {
  core::Box box; box.periodic = {true, true, true};
  core::AtomSoA<double> a;
  ASSERT_TRUE(io::read_lammps_data(ters_dir() + "tersoff_rdf_216.data", a, box));
  ASSERT_EQ(a.n, 216);

  pot::TersoffParams p;
  const double rmax = p.rcut();  // LAMMPS compute-rdf default cutoff = the Tersoff cutoff R+D=3.2
  const core::PairGeom geom(box, rmax);
  const int frames = 16;
  const long per = 40;
  ...
  for (int f = 0; f < frames; ++f) {
    serial_vv_tersoff(a, geom, p, per, dt);
    accum_hist(a, box, rmax, hist);
  }
  const auto g_our = normalize_gr(hist, frames + 1, a, box, rmax);
  const auto g_lmp = load_lammps_gr(ters_dir() + "tersoff_rdf_lammps.gr");
  ...
  std::printf("[Tersoff RDF] cosine overlap = %.6f | nn-peak ours=%.3f lammps=%.3f\n", cos, pk_our, pk_lmp);
